...
import { OUTCOME_NATURAL_WIN, OUTCOME_CRAPS_LOSS, OUTCOME_POINT_WIN, OUTCOME_SEVEN_OUT, RESULT_WON, RESULT_LOST } from "./constants";
...
export const useCrapsGame = () => {
  ...
  const [lastResult, setLastResult] = useState<typeof RESULT_WON | typeof RESULT_LOST | null>(null);

  const handleComeOut = (d1: number, d2: number, sum: number, newRollCount: number): GameRollResult => {
  ...
  const handlePointRoll = (d1: number, d2: number, sum: number, newRollCount: number): GameRollResult => {
